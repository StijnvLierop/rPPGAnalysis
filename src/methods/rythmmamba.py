...
class Fusion_Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        x_diff = self.stem12(
            torch.cat([x2 - x1, x3 - x2, x4 - x3, x5 - x4], 2).view(N * D, 12, H, W))
...
class Block_mamba(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, segment):
            for j in range(i):
                x_o[0:B, tt * i: tt * (i + 1), :] = x_o[0:B, tt * i: tt * (i + 1), :] + x_o[B * (
                            j + 1):B * (j + 2), tt * (i - j - 1): tt * (i - j), :]
...
class RhythmMamba(nn.Module):
    def __init__(self,
        ...
        self.stem3 = nn.Sequential(
            nn.Conv3d(embed_dim // 4, embed_dim, kernel_size=(2, 5, 5), stride=(2, 1, 1),
                      padding=(0, 2, 2)),
            nn.BatchNorm3d(embed_dim),
        )

        dpr = [x.item() for x in
               torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        inter_dpr = [0.0] + dpr
...
def rhythmmamba(frames: np.ndarray) -> np.ndarray:
    """
    Extracts the BVP signal using the RhythmMamba method.
